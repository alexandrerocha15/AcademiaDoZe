...
public class AlunoTests
{
    ...
    {
        DateOnly? data = scenario == "default"
            ? default(DateOnly?)
            : DateOnly.FromDateTime(DateTime.Today.AddYears(-10));

        var dataParam = data.HasValue
            ? data.Value
            : default(DateOnly);
        ...
        Assert.Contains(
            result.Notifications,
            n => n.Mensagem == expectedMessage);
    }

    [Theory(DisplayName = "Aluno: telefone obrigatório -> TELEFONE_OBRIGATORIO")]
    [InlineData(null)]
    [InlineData("")]
    public void Deve_Falhar_Criacao_Quando_TelefoneVazio(string? telefone)
    {
        var result = Aluno.Criar(
            1,
            "João",
    ...
    [Theory(DisplayName = "Aluno: CPF obrigatório -> CPF_OBRIGATORIO")]
    [InlineData(null)]
    [InlineData("")]
    public void Deve_Falhar_Criacao_Quando_CpfVazio(string? cpf)
    {
        var result = Aluno.Criar(
            1,
            "João",
    ...
    [Theory(DisplayName = "Aluno: senha obrigatória -> SENHA_OBRIGATORIO")]
    [InlineData(null)]
    [InlineData("")]
    public void Deve_Falhar_Criacao_Quando_SenhaVazia(string? senha)
    {
        var result = Aluno.Criar(
            1,
            "João",
