...
public class ColaboradorInfrastructureTests : TestBase
{
        ...
        LogradouroRepository logradouroRepo)
    {
        ...
        {
            throw new Exception(
                $"Falha ao criar Colaborador: {string.Join(", ", colaboradorResult.Notifications.Select(n => n.Mensagem))}");
        }
    ...
    public async Task Colaborador_Atualizar_Sucesso()
    {
        ...
        var colaborador = await _colaboradorRepo.Adicionar(
            Colaborador.Criar(
                0,
                "Alexandre",
                GerarCpf(),
    ...
    public async Task Colaborador_ObterPorVinculo_FiltragemCorreta()
    {
        ...
        var resultados =
            await _colaboradorRepo.ObterPorVinculo(colaborador.Vinculo);
