        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            string query = FormatInsertQuery(
                ...
                VALUES
                (@Cpf, @Nome, @Nascimento, @Telefone, @Email, @LogradouroId, @Numero, @Complemento, @Senha, @Foto)");
            ...
            command.AddParameter(
                "@Complemento",
                (object?)entity.Endereco.Complemento ?? DBNull.Value,
                DbType.String);
            command.AddParameter(
                "@Senha",
                entity.Senha.Valor,
...
WHERE id_aluno = @Id";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException(
                "ERRO_OBTER_POR_CPF",
                $"Erro ao obter aluno por CPF {cpf.Valor}: {ex.Message}",
                ex);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException(
                ...
                $"Erro ao obter aluno por Email {email.Valor}: {ex.Message}",
                ex);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "SELECT COUNT(1) FROM tb_aluno WHERE cpf = @Cpf AND (@Id IS NULL OR id_aluno <> @Id)";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "SELECT COUNT(1) FROM tb_aluno WHERE email = @Email AND (@Id IS NULL OR id_aluno <> @Id)";
