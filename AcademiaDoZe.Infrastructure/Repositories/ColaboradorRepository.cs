...
public class ColaboradorRepository : BaseRepository, IColaboradorRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            string query = FormatInsertQuery(
                ...
                (cpf, nome, nascimento, telefone, email, logradouro_id, numero, complemento, senha, foto, admissao, tipo, vinculo)
                VALUES
                (@Cpf, @Nome, @Nascimento, @Telefone, @Email, @LogradouroId, @Numero, @Complemento, @Senha, @Foto, @Admissao, @Tipo, @Vinculo)");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "SELECT COUNT(1) FROM tb_colaborador WHERE cpf = @Cpf AND (@Id IS NULL OR id_colaborador <> @Id)";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "SELECT COUNT(1) FROM tb_colaborador WHERE email = @Email AND (@Id IS NULL OR id_colaborador <> @Id)";
