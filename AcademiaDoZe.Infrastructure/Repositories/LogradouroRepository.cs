...
public class LogradouroRepository : BaseRepository, ILogradouroRepository
{
        ...
        try
        {
            ...
            if (result.IsFailure)
            {
                throw new InfrastructureException(
                    ...
                    $"Erro de domínio ao mapear logradouro ID {id}: {string.Join(", ", result.Notifications.Select(n => n.Mensagem))}");
            }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            string query = FormatInsertQuery(
                "INSERT INTO tb_logradouro (cep, nome, bairro, cidade, estado, pais) VALUES (@Cep, @Nome, @Bairro, @Cidade, @Estado, @Pais)");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "UPDATE tb_logradouro SET cep = @Cep, nome = @Nome, bairro = @Bairro, cidade = @Cidade, estado = @Estado, pais = @Pais WHERE id_logradouro = @Id";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "DELETE FROM tb_logradouro WHERE id_logradouro = @Id";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                "SELECT COUNT(1) FROM tb_logradouro WHERE cep = @Cep AND (@Id IS NULL OR id_logradouro <> @Id)";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                $"{BaseSelectQuery} WHERE cidade = @Cidade ORDER BY bairro, nome";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                $"{BaseSelectQuery} WHERE cidade = @Cidade AND bairro = @Bairro ORDER BY nome";
