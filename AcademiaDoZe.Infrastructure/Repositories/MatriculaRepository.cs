...
public class MatriculaRepository : BaseRepository, IMatriculaRepository
{
    ...
    private static string BaseSelectQuery => """
        SELECT 
            ...
            l.id_logradouro, l.cep, l.nome AS logradouro_nome, l.bairro,
    ...
    public static Matricula Map(DbDataReader reader)
    {
        try
        {
            ...
            Arquivo? laudoMedico = laudoBytes != null
                ? Arquivo.Criar(laudoBytes).Value
                : null;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            command.AddParameter(
                "@RestricaoMedica",
                (int)entity.RestricoesMedicas,
                DbType.Int32);

            command.AddParameter(
                "@ObsRestricao",
                (object?)entity.ObservacoesRestricoes ?? DBNull.Value,
                DbType.String);
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException(
                "ERRO_ADICIONAR_MATRICULA",
                $"Erro ao adicionar matrícula para o aluno ID {entity.AlunoId}: {ex.Message}",
                ex);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (rowsAffected == 0)
            {
                throw new InfrastructureException(
                    ...
                    $"Nenhuma matrícula encontrada com o ID {entity.Id} para atualização.");
            }
        ...
        catch (DbException ex)
        {
            throw new InfrastructureException(
                ...
                $"Erro ao atualizar matrícula ID {entity.Id} do aluno ID {entity.AlunoId}: {ex.Message}",
                ex);
        }
    }
    ...
    {
        var matricula =
            await ObterMatriculaAtivaPorAluno(alunoId, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                $"{BaseSelectQuery} WHERE m.data_fim >= {GetCurrentDateFunction()} " +
                $"{(alunoId > 0 ? "AND m.aluno_id = @id" : "")} " +
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            string maxDate =
                GetDateAddDaysExpression(currentDate, "@Dias");

            string query =
                $"{BaseSelectQuery} WHERE m.data_fim >= {currentDate} " +
                $"AND m.data_fim <= {maxDate} ORDER BY m.data_fim ASC";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string query =
                $"{BaseSelectQuery} WHERE m.plano = @Plano ORDER BY a.nome";
